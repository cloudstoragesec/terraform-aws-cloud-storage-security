...
resource "aws_appconfig_environment" "agent" {
  name           = "${var.service_name}Env-${local.app_id}"
  description    = "AppConfig Environment for CloudStorageSec Agents"
  application_id = local.app_id
  tags = merge({ (join("-", ["${var.service_name}", "${local.app_id}"])) = "ConfigEnvironment" },
    var.custom_resource_tags
  )
}

resource "aws_appconfig_deployment_strategy" "agent" {
  name                           = "${var.service_name}ConfigDeploy-${local.app_id}"
  ...
  tags = merge({ (join("-", ["${var.service_name}", "${local.app_id}"])) = "ConfigStartegy" },
    var.custom_resource_tags
  )
}

resource "aws_ssm_document" "appconfig_document_schema" {
  name            = "${var.service_name}Config-Schema-${local.app_id}"
  document_type   = "ApplicationConfigurationSchema"
  document_format = "JSON"
  content         = file("${path.module}/appconfig_schema.json")

  tags = merge({ (join("-", ["${var.service_name}", "${local.app_id}"])) = "ConfigSchema" },
    var.custom_resource_tags
  )
}

resource "awscc_ssm_document" "appconfig_document" {
  name            = "${var.service_name}Config-Doc-${local.app_id}"
  ...
  tags = concat([
    {
      key   = (join("-", ["${var.service_name}", "${local.app_id}"]))
      value = "ConfigDoc"
    },
...
resource "aws_appconfig_configuration_profile" "agent" {
  ...
  name               = "${var.service_name}Config-Profile-${local.app_id}"
  ...
  tags = merge({ (join("-", ["${var.service_name}", "${local.app_id}"])) = "ConfigProfile" },
    var.custom_resource_tags
  )
...
resource "aws_appconfig_deployment" "agent" {
  ...
  tags = merge({ (join("-", ["${var.service_name}", "${local.app_id}"])) = "ConfigDeployment" },
    var.custom_resource_tags
  )
}
