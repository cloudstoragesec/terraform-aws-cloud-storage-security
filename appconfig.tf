...
resource "aws_appconfig_environment" "agent" {
  ...
  tags = merge({ (join("-", ["${var.service_name}", "${local.application_id}"])) = "ConfigEnvironment" },
    var.custom_resource_tags
  )
}
...
resource "aws_appconfig_deployment_strategy" "agent" {
  ...
  tags = merge({ (join("-", ["${var.service_name}", "${local.application_id}"])) = "ConfigStartegy" },
    var.custom_resource_tags
  )
}
...
resource "aws_ssm_document" "appconfig_document_schema" {
  ...
  tags = merge({ (join("-", ["${var.service_name}", "${local.application_id}"])) = "ConfigSchema" },
    var.custom_resource_tags
  )
}
...
resource "awscc_ssm_document" "appconfig_document" {
  ...
  tags = concat([
    {
      key   = (join("-", ["${var.service_name}", "${local.application_id}"]))
      value = "ConfigDoc"
    },
...
resource "aws_appconfig_configuration_profile" "agent" {
  ...
  tags = merge({ (join("-", ["${var.service_name}", "${local.application_id}"])) = "ConfigProfile" },
    var.custom_resource_tags
  )
...
resource "aws_appconfig_deployment" "agent" {
  ...
  tags = merge({ (join("-", ["${var.service_name}", "${local.application_id}"])) = "ConfigDeployment" },
    var.custom_resource_tags
  )
}
